...
eq(SPS.基礎控除(4890000, 'r8'), 1040000, '令和8・9年分の基礎控除：489万円以下は104万円');
 ...
 *
 *   住民税がかからない線（一次資料）:
 *     ・ひとり親で合計所得135万円以下 … 地方税法 第295条第1項第2号
 *     ・均等割 35万円×(本人+扶養)+10万円+21万円 … 地方税法施行令 第47条の3
 *     ・所得割 35万円×(本人+扶養)+10万円+32万円 … 地方税法 附則第3条の3第4項
 * ---------------------------------------------------------- */
見出し('7-2. ひとり親控除で軽くなる税');

/* 所得税で35万円、住民税で30万円を所得から引ける（データと一致していること） */
var 控除表 = データ.programs_by_id.hitorioya_kojo.eligibility;
eq(控除表.deduction_income_tax, 350000, '所得税から引ける額は35万円');
eq(控除表.deduction_resident_tax, 300000, '住民税から引ける額は30万円');

/* 収入帯ごとの理論値。ずれは1円まで許す（円未満の切り捨てが入るため） */
function ちかい(a, b, name, 許容) {
  ok(Math.abs(a - b) <= (許容 || 1), name, '出た値 ' + a + ' / 理論値 ' + b);
}
var 子1人 = 1;

/* ここからの金額は、令和8年分・9年分の表（給与所得控除の最低保障74万円、基礎控除104万円）で出す。 */
/* 年収150万円: 給与所得76万円。ひとり親でなくても所得税は0円、
   住民税も非課税の線（子1人なら112万円）を下回るので0円。よって効果も0円。 */
...
/* 年収180万円: 給与所得106万円。ひとり親でなくても所得割の線（子1人なら112万円）を下回るが、
   均等割の線（101万円）は上回るので、均等割5,000円だけがかかる。
   ひとり親なら135万円以下で住民税は丸ごと0円。よって効果は5,000円。
   所得税はどちらも0円（基礎控除を引くと課税所得が残らない）。 */
var k180 = SPS.ひとり親控除の効果(1800000, 子1人);
eq(k180.incomeTax, 0, '年収180万円では、所得税はもともと0円なので効果も0円');
eq(k180.residentTax, 5000, '年収180万円では、均等割5,000円がかからなくなる');

/* 年収204.4万円: 給与所得130.4万円。ひとり親なら135万円以下で住民税は丸ごと0円。
   ひとり親でなければ所得割もかかるので、その全額が「効果」になる。 */
var k204 = SPS.ひとり親控除の効果(2044000, 子1人);
eq(k204.incomeTax, 0, '年収204.4万円では、所得税はもともと0円なので効果も0円');
ok(k204.residentTax > 300000 * 0.10,
  '年収204.4万円では、住民税が丸ごとかからなくなるので、30万円×10%より効果が大きい');

/* 年収280万円・320万円・450万円:
   所得税の課税所得が35万円より多く残り、税率は5%の帯。
   効果 ＝ 35万円×5%×1.021（所得税） ＋ 30万円×10%（住民税） */
...
[1500000, 2100000, 2600000, 3200000, 4500000, 6000000].forEach(function (年収) {
  var v = SPS.ひとり親控除の効果(年収, 子1人).total;
  /* 課税所得を千円単位に切りそろえるので、1円だけ上下することがある */
  ok(v >= 前の効果 - 1, '年収' + (年収 / 10000) + '万円まで、効果が逆戻りしない');
  前の効果 = v;
});

/* 制度カードとグラフが、同じ関数の数字を使っていること */
var カード = SPS.制度判定({
  isSingleParent: true, myIncome: 1500000, otherIncome: 0, spouseIncome: 0,
  children: [{ age: 5 }], currentSavings: 300000, livingCost: 150000
}, データ).results.filter(function (r) { return r.program.id === 'hitorioya_kojo'; })[0];
eq(カード.taxSaving.total, 0, '年収150万円のカードでも、軽くなる税は0円');
ok(カード.amountText.indexOf('0円') >= 0, '0円のときは、カードにも0円と出る');
ok(カード.note.indexOf('もともと所得税・住民税がかかりません') > 0,
  '0円のときは、なぜ0円なのかがカードに書いてある');
ok(カード.note.indexOf('それでも申告しておく意味があります') > 0,
  '0円でも申告する意味が書いてある');
var カード高 = SPS.制度判定({
  isSingleParent: true, myIncome: 3200000, otherIncome: 0, spouseIncome: 0,
  children: [{ age: 5 }], currentSavings: 300000, livingCost: 150000
}, データ).results.filter(function (r) { return r.program.id === 'hitorioya_kojo'; })[0];
eq(カード高.taxSaving.total, SPS.ひとり親控除の効果(3200000, 1).total,
  'カードの数字は、共通の計算から出ている');

/* 住民税そのものの線も固定しておく（合計所得で見る） */
eq(SPS.住民税額(1300000, 188500, 1, true), 0,
  'ひとり親で合計所得135万円以下なら、住民税はかからない');
ok(SPS.住民税額(1400000, 203000, 1, true) > 0,
  'ひとり親でも、合計所得が135万円をこえれば住民税はかかる');
/* ひとり親でない場合は、線が2本ある。
   均等割の線（子1人なら101万円）と、所得割の線（子1人なら112万円）。 */
...
/* 生活防衛資金の線は、グラフには描かない（見せたい線が読みにくくなるため）。
   目標の額そのものは、月ごとに持ちつづける（グラフの下の文章で使う）。 */
ok(月svg.indexOf('生活防衛資金') === -1 && 月svg.indexOf('#1c7a4a') === -1,
  'グラフに、生活防衛資金の線も文字も出ていない');
var 目標の列 = 月c.monthly.map(function (q) { return q.target; });
...
/* 双子は同じ順位にしない（issue #7） */
eq(SPS.保育料([1, 1], 4500000, true, 保), 30000 + 15000, '双子は、1人目が全額・2人目が半額');
eq(SPS.保育料([1, 1], 3000000, true, 保), 9000, '年収360万円未満のひとり親の双子は、2人目が0円');
eq(SPS.保育料([1, 1, 1], 4500000, true, 保), 30000 + 15000 + 0, '三つ子は、3人目が0円');
eq(SPS.保育料([4, 1, 1], 4500000, true, 保), 15000 + 0, '上に4歳の子がいる双子は、2人目が半額・3人目が0円');
...
/* 区分の境目は、年収の決め打ちではなく、国の算式で出す。
   文部科学省の資料6の目安年収（ひとり親世帯・母のみが生計維持者）と突き合わせる。
   社会保険料を14.5%と置いた見積もりなので、目安とのずれは10万円まで許す。 */
function 境目の年収(年齢たち, 人数) {
  var 出 = [], 前 = null;
  for (var x = 1500000; x <= 7500000; x += 10000) {
    var r = SPS.大学の支援割合(x, 人数, データ.tuition.support, 年齢たち).ratio;
    if (前 !== null && r !== 前) { 出.push(x - 10000); }
    前 = r;
  }
  return 出;
}
[
  { 名: '子1人（本人）', 年齢: [18], 目安: [2100000, 3000000, 3700000] },
  { 名: '子2人（本人・高校生）', 年齢: [18, 17], 目安: [2700000, 3600000, 4300000] },
  { 名: '子3人（本人・高校生・中学生）', 年齢: [18, 17, 14], 目安: [2700000, 3600000, 4300000] },
  { 名: '子3人（本人・大学生・高校生）', 年齢: [18, 20, 17], 目安: [3500000, 4500000, 5100000] }
].forEach(function (k) {
  var 境 = 境目の年収(k.年齢, k.年齢.length);
  [0, 1, 2].forEach(function (n) {
    ok(Math.abs(境[n] - k.目安[n]) <= 100000,
      'ひとり親・' + k.名 + 'の第' + ['Ⅰ', 'Ⅱ', 'Ⅲ'][n] + '区分の上限が、国の目安（約' + (k.目安[n] / 10000) + '万円）と合う',
      '出た値 ' + (境[n] / 10000) + '万円');
  });
});
eq(SPS.学費の支援(19, { university: 'national_home' }, データ.tuition, 状況(2000000, 2, true)),
  535800 + 350400, '年収200万円（住民税非課税）は満額');
ok(SPS.学費の支援(19, { university: 'national_home' }, データ.tuition, 状況(3000000, 2, true)) <
   535800 + 350400, '年収300万円は、子2人のひとり親だと満額ではない（以前は満額にしていて甘かった）');
eq(SPS.学費の支援(19, { university: 'national_home' }, データ.tuition, 状況(4500000, 2, true)), 0,
  '年収450万円・子2人のひとり親は、支援なし（国の目安は約430万円まで）');
/* お子さん3人以上（多子世帯） */
...
ok(奨学文.indexOf('「返済不要の給付型奨学金」だけ') > 0, '返さなくていいものだけを探す指示が入っている');
/* コピーして使う文章なので、HTMLのタグが混ざっていてはいけない */
文章.forEach(function (p) {
  ok(!/<\/?[a-zA-Z][a-zA-Z0-9]*>/.test(p.text),
    '「' + p.title + '」の文章に、HTMLのタグが混ざっていない');
});
...
console.log('\n== 15. 使い方マニュアルと中身の食い違い ==');
(function () {
  var man = fs.readFileSync(path.join(ROOT, 'manual.html'), 'utf8');
  var eng = fs.readFileSync(path.join(ROOT, 'js', 'engine.js'), 'utf8');

  ok(eng.indexOf('入力.currentSavings') > 0, '貯金は、入力した額を出発点にしている');
  ok(man.indexOf('0円から始まる') === -1,
    'マニュアルのどこにも「0円から始まる」と書かれていない');
  ...
  ok(eng.indexOf('減免割合') > 0, '授業料の減免を計算に入れている');
  ok(man.indexOf('学費は満額かかる前提で描いています') === -1,
    'マニュアルに「学費は満額かかる前提」と書かれていない');
})();
