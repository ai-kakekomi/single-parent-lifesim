...
(function () {
  ...
  function 資産を描く() {
    ...
    /* 「何をすればいいか」は、貯金の項目には置かない。制度を探す項目にまとめる。
       貯金の項目は、見通しを見るところ。打つ手が2か所に分かれていると、同じ話が重なって読みにくい。 */
    $('stage1-path').innerHTML = 道筋を描く(道筋(入力, データ, c, 最新判定)) + 赤字の警告(c);
  ...
  function 打ち切りの注記(c) {
    ...
    var h = '<p class="hint floor-note">お金が足りなくなっても、貸金業者（消費者金融や、クレジットカードのキャッシング）から借りられるのは、' +
      ...
      '<a href="#gap-block">足りないぶんを埋めるために、今週から動けることを見る</a><br>' +
  ...
  function 赤字の警告(c) {
    ...
    /* 養育費・まだ申請していない制度・資格・生活保護は、上の「あなたの場合の道筋」に
    ...
    h.push('<h4>🔴 足りないぶんを埋めるために、今週から動けること</h4>');
    if (不足 > 0) {
    ...
    h.push('<p><strong>借金では埋められません。</strong>カードローンやリボ払いで足りないぶんを埋めると、' +
      '来月からは返済も足されて、もっと足りなくなります。上の道筋に加えて、下のことも試してください。</p>');
