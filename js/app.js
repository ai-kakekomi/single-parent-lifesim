...
(function () {
  ...
  function 見方の説明() {
    if (グラフの見方 === 'total') {
      return '<p class="hint">いま見ているのは <strong>家全体</strong>の金額です。' +
        '結婚を続けた場合は大人2人分、離婚した場合は大人1人分の暮らしなので、この数字をそのまま比べると、' +
  ...
  function 資産を描く() {
    ...
    if (c.gaps.length) {
      ...
      伸びしろ = '<div class="headline-box ok">' +
        ...
        '<p>このアプリが自動で判定できる制度は、すでに使えているようです。' +
        'あとは市区町村ごとの制度が残っています。' +
        '<a href="#stage1">確認したい制度の一覧を見る</a></p></div>';
    }
    ...
    $('stage2b-body').innerHTML =
      ...
      /* ここから下は、詳しく知りたい人のための折りたたみ。一番下にまとめておく */
      資格ルートの説明(c) +
      学費の説明(c) +
      奨学金の見取り図(c) +
      打ち切りの注記(c) +
  ...
  function 前提のボックス(c) {
    ...
    h.push('<li><strong>学校にかかるお金は、全国の平均値です。</strong>まん中の人の金額ではありません。' +
      ...
      'また、1年分の金額を<strong>12か月に等分して</strong>引いています。' +
    ...
    h.push('<li><strong>グラフに入れているのは、返さなくていいお金だけです。</strong>' +
      ...
      '借りれば一時的に貯金は増えますが、あとで返す分、実際には楽になっていないからです。</li>');
  ...
  function 線の本数の注記(c) {
    ...
    return '<p class="hint"><strong>制度はすでに使いきっています。線は1本です。</strong>' +
      'このアプリが自動で判定できる制度に、取りこぼしは見あたりませんでした。</p>';
  }
    ...
    b.expense.forEach(function (r) {
      ...
      if (r.key === 'living' && r.increase > 0) {
        追記 = '<span class="why">はじめの ' + SPS.円(r.baseline) +
          ' ＋ お子さんの成長で増える食費分 ' + SPS.円(r.increase) + '</span>';
      }
      if (r.key === 'childcare') {
        if (r.discount > 0) {
  ...
  /* ---------- 不足分を、どうやって埋めるか ----------
     「崩れます」で終わらせず、その分をどこから持ってくるかの一覧を出す。 */
  /* ============================================================
   * 奨学金の見取り図
  ...
  function 奨学金の見取り図(c) {
    ...
    h.push('<tr><th>高校</th>' +
      ...
      '<br><strong>高校生等奨学給付金</strong><br><span class="why">所得で決まります。このアプリで計算ずみ</span></td>' +
    ...
    h.push('<tr><th>大学など</th>' +
      ...
      '所得で決まります。このアプリで計算ずみ</span>' +
      '<br><strong>大学独自・民間の給付型</strong><br><span class="why">数千種類あるのでこのアプリには入れられません。' +
    ...
    if (c.universityShortfall > 0) {
      ...
      if (返) {
        ...
        h.push('<p class="loan-sim-head">不足分を、貸与型で埋めた場合</p>');
  ...
  function チェックリストを描く(一覧) {
    ...
    var h = ['<div class="checklist">',
      '<p class="hint" style="margin-top:0">上から順に、ひとつずつで大丈夫です。</p>',
      '<ul>'];
  ...
  function 身の安全を描く() {
    var p = データ.programs_by_id.dv_support;
    if (!p) { return; }
    var e = p.eligibility || {};
    var h = [];
    h.push('<p class="safety-lead"><strong>' + esc(p.summary) + '</strong></p>');
    h.push('<p class="safety-tel">' + esc(p.how_to_apply) + '</p>');
    h.push('<p>' + esc(p.benefit_summary) + '</p>');
    h.push('<ul>' + (p.cautions || []).map(function (c) {
      return '<li>' + esc(c) + '</li>';
    }).join('') + '</ul>');
    if (e.shien_sochi) { h.push('<p class="hint">' + esc(e.shien_sochi) + '</p>'); }
    h.push('<p class="src">出典: <a href="' + esc(p.source.url) + '" target="_blank" rel="noopener">' +
      esc(p.source.publisher) + '</a>（最終確認 ' + 日付表示(p.source.last_verified) + '）</p>');
  ...
  function まとめを描く() {
    ...
    h.push('<p class="hint">この画面を閉じると、入力した内容もチェックも消えます。' +
  ...
  var メニューの項目 = [
    ...
    { id: 'programs', 絵: '🔍', 名: 'ひとり親支援制度を探す', 章: ['stage3a', 'stage1'] },
