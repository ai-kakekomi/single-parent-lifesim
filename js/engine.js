...
(function (root, factory) {
   ...
   *   これからの手取りのめやすには 'r8'（令和8年分・9年分、最低保障額 74万円）を使う。
   ...
   *   つまり東京23区などと同じ、いちばん有利な線で計算しています。
   * ---------------------------------------------------------- */
  var 住民税表 = {
    ひとり親等の非課税限度: 1350000,
    基本額: 350000,
  ...
  function 手取りめやす(額面年収, ひとり親か, 扶養の数) {
  ...
  function 手取りの補正(入力) {
    ...
    return 明細 - Math.floor(手取りめやす(額面, 控除込み, 扶養) / 12);
  }
  ...
  function シミュレーション(入力, データ) {
    ...
    var 自分手取り月 = Math.floor(手取りめやす(入力.myIncome, true, (入力.children || []).length) / 12) + 補正;
    var 婚姻中手取り月 = Math.floor(手取りめやす(入力.myIncome, false) / 12) + 補正
  ...
  function 資産カーブ(入力, データ) {
    ...
    /* ひとり親控除で軽くなる税（ひと月あたり）。カードと同じ1本の関数を使う */
          ...
          if (入力.isSingleParent && 合計所得 <= e.income_ceiling && (入力.children || []).length > 0) {
            ...
            if (効果 === 0) {
              ...
              r.note = 'いまの収入では、この控除がなくても、もともと所得税・住民税がかかりません。'
                ...
                + '合計所得が135万円以下のあいだは住民税がかからない扱いになり（地方税法第295条第1項第2号）、'
