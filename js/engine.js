...
(function (root, factory) {
   ...
   *   （最終確認日 2026-09-21）
   *
   *   既定は 'r7'（令和7年分・最低保障額 65万円）。
   *   児童扶養手当や住民税非課税の判定は「前年の所得」で決まるので、
   *   いま（令和8年度）の判定には令和7年分の表を使う。
   *   これからの手取りの目安には 'r8'（令和8年分・9年分、最低保障額 74万円）を使う。
   *   令和10年分以後の表は国税庁のページにまだ載っていないので入れていない。
   * ---------------------------------------------------------- */
  var 給与所得控除表 = {
    r8: [ /* 令和8年分・令和9年分 */
      { upto: 2200000, calc: function () { return 740000; } },
  ...
  /* ------------------------------------------------------------
   * 5. 手取りの目安（表示のためだけの概算。判定には一切使わない）
   *
   ...
   *     住民税     ＝ 所得割 ＋ 均等割
   *        ただし、住民税には「ここまでなら税がかからない」線が3本あります。
   *        どれかに当てはまれば住民税は0円です（下の 住民税額 を見てください）。
   *
   *   扶養している子の扶養控除（16歳以上）や配偶者控除、医療費控除などは入れていません。
   *   あくまで画面表示のための目安で、実際の手取りとはずれます。
   * ---------------------------------------------------------- */
  /* 国税庁 No.1199「基礎控除」（最終確認日 2026-09-21）
     https://www.nta.go.jp/taxes/shiraberu/taxanswer/shotoku/1199.htm */
  var 手取りの年分 = 'r8';
  var 基礎控除の高所得部分 = [
    { upto: 24000000, v: 480000 },
    { upto: 24500000, v: 320000 },
    { upto: 25000000, v: 160000 },
    { upto: Infinity, v: 0 }
  ];
  var 基礎控除表 = {
    r8: [ /* 令和8年分・令和9年分 */
      { upto: 4890000, v: 1040000 },
      { upto: 6550000, v: 670000 },
      { upto: 23500000, v: 620000 }
    ].concat(基礎控除の高所得部分),
    r7: [ /* 令和7年分 */
      { upto: 1320000, v: 950000 },
      { upto: 3360000, v: 880000 },
      { upto: 4890000, v: 680000 },
      { upto: 6550000, v: 630000 },
      { upto: 23500000, v: 580000 }
    ].concat(基礎控除の高所得部分)
  };
  function 基礎控除(合計所得, 年分) {
    var 表 = 基礎控除表[年分 || 手取りの年分];
    for (var i = 0; i < 表.length; i++) {
      if (合計所得 <= 表[i].upto) { return 表[i].v; }
    }
    return 0;
  }
  ...
  /* ------------------------------------------------------------
   * 住民税が「かからない」線
   *   ひとり親控除は、税を払っている人にしか効きません。
   *   もともと税がかからない収入なら、控除で軽くなる額は0円です。
   *   そこを正しく出すために、非課税の線を条文どおりに置きます。
   *
   *   ・ひとり親（寡婦・障害者・未成年も同じ）で合計所得135万円以下 → 住民税はかからない
   *       地方税法 第295条第1項第2号
   *   ・均等割がかからない線 ＝ 35万円 ×（本人＋扶養の数）＋ 10万円
   *                            ＋（扶養がいれば 21万円）
   *       地方税法施行令 第47条の3
   *   ・所得割がかからない線 ＝ 35万円 ×（本人＋扶養の数）＋ 10万円
   *                            ＋（扶養がいれば 32万円）
   *       地方税法 附則 第3条の3第4項
   *
   *   35万円と21万円は、お住まいの地域（生活保護の級地区分）によって
   *   これより低いことがあります。ここでは条文の上限値、
   *   つまり東京23区などと同じ、一番有利な線で計算しています。
   * ---------------------------------------------------------- */
  var 住民税表 = {
    ひとり親等の非課税限度: 1350000,
    基本額: 350000,
  ...
  /** 住民税の年額の目安（合計所得と社会保険料から出す） */
  function 住民税額(合計所得, 社会保険料, 扶養の数, ひとり親か) {
    var 人数 = 1 + Math.max(0, Math.floor(扶養の数 || 0));
    var 扶養あり = Math.floor(扶養の数 || 0) > 0;
    var t = 住民税表;
    /* ひとり親なら、合計所得135万円以下は、まるごとかからない */
    if (ひとり親か && 合計所得 <= t.ひとり親等の非課税限度) { return 0; }
    var 均等割の線 = t.基本額 * 人数 + t.上乗せ + (扶養あり ? t.加算_均等割 : 0);
    var 所得割の線 = t.基本額 * 人数 + t.上乗せ + (扶養あり ? t.加算_所得割 : 0);
    var 均等割 = (合計所得 <= 均等割の線) ? 0 : t.均等割;
    var 課税所得 = Math.max(0, 合計所得 - 社会保険料 - t.基礎控除
      - (ひとり親か ? t.ひとり親控除 : 0));
    var 所得割 = (合計所得 <= 所得割の線) ? 0 : Math.floor(課税所得 * t.所得割の率);
    return 均等割 + 所得割;
  }

  /** 所得税の年額の目安 */
  function 所得税の額(合計所得, 社会保険料, ひとり親か) {
    return 所得税額(Math.max(0, 合計所得 - 社会保険料 - 基礎控除(合計所得)
      - (ひとり親か ? 350000 : 0)));
  }

  function 社会保険料(額面) { return Math.floor(額面 * 0.145); }

  function 手取り目安(額面年収, ひとり親か, 扶養の数) {
    var 額面 = Math.max(0, Math.floor(額面年収 || 0));
    if (額面 === 0) { return 0; }
    var 社保 = 社会保険料(額面);
    var 所得 = 給与所得(額面, 手取りの年分);
    return Math.max(0, 額面 - 社保
      - 所得税の額(所得, 社保, ひとり親か)
      - 住民税額(所得, 社保, 扶養の数, ひとり親か));
  }

  /* ------------------------------------------------------------
   * 手取りの補正（1か月あたり・円。マイナスもある）
   *
   *   社会保険料は「会社の社会保険に入っている」前提の一律14.5%で見積もっている。
   *   国民健康保険・国民年金の人、保険に入っていない人は、これと大きくずれる。
   *   国保の料率は市区町村ごとに違うので、表を持つのはやめて、
   *   給与明細の手取りを入れてもらい、見積もりとの差を「補正」として持つ。
   *
   *   手取りそのものではなく差で持つのは、「年収を20万円ふやしたら」のように
   *   年収を動かして比べる計算でも、増えた分の手取りが正しく出るようにするため。
   *   判定（児童扶養手当など）には使わない。判定は額面の年収で行う。
   *
   * @param {Object} 入力 { takeHomeMonthly, myIncome, isSingleParent, usedPrograms, children }
   * ---------------------------------------------------------- */
  function 手取りの補正(入力) {
    var 明細 = Math.floor((入力 && 入力.takeHomeMonthly) || 0);
    var 額面 = Math.floor((入力 && 入力.myIncome) || 0);
    /* 空欄・額面より多い手取り（入れ間違い）は、補正しない */
    if (明細 <= 0 || 額面 <= 0 || 明細 * 12 > 額面) { return 0; }
    /* いまの明細に、ひとり親控除がもう効いているか */
    var 控除込み = !!入力.isSingleParent
      && (入力.usedPrograms || []).indexOf('hitorioya_kojo') >= 0;
    var 扶養 = 入力.isSingleParent ? (入力.children || []).length : 0;
    return 明細 - Math.floor(手取り目安(額面, 控除込み, 扶養) / 12);
  }

  /* ------------------------------------------------------------
   * ひとり親控除で、1年に軽くなる税額
   *   カードもグラフも、かならずこの1本の関数を通します。
   *   （計算の道が2本あると、画面のどこかで数字が食い違うため）
   * ---------------------------------------------------------- */
  function ひとり親控除の効果(額面年収, 扶養の数) {
    var 額面 = Math.max(0, Math.floor(額面年収 || 0));
    if (額面 === 0) { return { total: 0, incomeTax: 0, residentTax: 0, taxFree: true }; }
    var 社保 = 社会保険料(額面);
    var 所得 = 給与所得(額面, 手取りの年分);
    var 所得税差 = 所得税の額(所得, 社保, false) - 所得税の額(所得, 社保, true);
    var 住民税差 = 住民税額(所得, 社保, 扶養の数, false)
      - 住民税額(所得, 社保, 扶養の数, true);
    return {
      total: Math.max(0, 所得税差 + 住民税差),
      incomeTax: Math.max(0, 所得税差),
      residentTax: Math.max(0, 住民税差),
      /* 控除がなくても、もともと税がかからない状態かどうか */
      taxFree: (所得税の額(所得, 社保, false) === 0
        && 住民税額(所得, 社保, 扶養の数, false) === 0)
    };
  }

  /* ------------------------------------------------------------
   * 5-2. 1人あたりに直した金額（等価可処分所得）
   *
   *   家族の人数が違う暮らしを、同じものさしで比べるための計算です。
   *
   *     1人あたりに直した金額 ＝ 1か月の合計 ÷ √世帯人数
   *
   *   ただ人数で割らないのは、電気代や家賃のように
   *   「人数が増えてもそれほど増えない費用」があるためです。
   ...
   *   （最終確認日 2026-08-11）
   * ---------------------------------------------------------- */
  function 等価所得(月の合計, 世帯人数) {
    var n = Math.max(1, Math.floor(世帯人数 || 1));
    return Math.round((月の合計 || 0) / Math.sqrt(n));
  }
   ...
   *   末子が22歳になる年まで、1年きざみで
   *     「結婚を続けた場合」と「離婚した場合」の
   *     1か月あたりの お金（手取り＋手当−住居費）を出す。
   *
   *   出す数字は2とおり
   *     ・total     ... 家全体で、家賃を払ったあと1か月に残るお金
   *     ・perPerson ... それを1人あたりに直した金額（÷√世帯人数）
  ...
  function シミュレーション(入力, データ) {
    ...
    /* 給与明細の手取りを入れてもらえたときは、見積もりとの差を足す（app.js が一度だけ出す） */
    var 補正 = Math.floor(入力.takeHomeAdjustMonthly || 0);
    var 自分手取り月 = Math.floor(手取り目安(入力.myIncome, true, (入力.children || []).length) / 12) + 補正;
    var 婚姻中手取り月 = Math.floor(手取り目安(入力.myIncome, false) / 12) + 補正
      + Math.floor(手取り目安(入力.spouseIncome, false) / 12);
   ...
   * 6-2. 貯金のたまり方（資産カーブ）
   *
   *   1か月の残り ＝ 手取り ＋ 手当 ＋ 養育費 ＋ 親の援助 − 住居費 − 生活費
   *   これを、一番下のお子さんが22歳になるまで、1か月ずつ積み上げます。
   *
   *   線は2本
   ...
   *
   *   入力された「いまの貯金額」を出発点にして積み上げます（入れていなければ0円から）。
   *   利息はつけません。赤字になる場合も、0で止めずにマイナスのまま描きます。
   *
   ...
   *
   * @param {Object} 塾設定 { useAverage: 全国平均を使うか, monthly: 1か月の額 }
   */
   ...
   * ---------------------------------------------------------- */
  /* 区分は、年収の決め打ちではなく、国の算式で出す。
   *
   *   支給額算定基準額 ＝ 市町村民税の所得割の課税標準額 × 6% −（調整控除の額 ＋ 税額調整額）
   *     第Ⅰ区分 100円未満（住民税が非課税の人は0円とみなす）／第Ⅱ区分 25,600円未満／
   *     第Ⅲ区分 51,300円未満／第Ⅳ区分（多子世帯）154,500円未満
   *   出典: 文部科学省「支援対象者の要件（個人要件）等 ＜所得に関する要件と目安年収＞」（資料6）
   *
   *   以前は、年収300万・400万・460万円で切っていた。これは、ふたり親で大学生と高校生がいる世帯の
   *   目安に近く、ひとり親には甘すぎた（国の目安は、ひとり親・子1人で約210万・300万・370万円、
   *   子2人（本人・高校生）で約270万・360万・430万円）。
   *   この算式に、社会保険料14.5%を置くと、国の目安年収（ひとり親の4つの例）とほぼ一致する。
   *
   *   扶養控除は年齢で変わる（16〜18歳は33万円、19〜22歳は45万円）。きょうだいの年齢で区分が動くので、
   *   その年の年齢を渡してもらう。年齢が分からないときは、国の例と同じ「本人18歳・ほかは高校生」で見る。
   * ---------------------------------------------------------- */
  var 修学支援の基準 = { 第1: 100, 第2: 25600, 第3: 51300, 第4: 154500 };
  function 大学の支援割合(年収, 子の人数, 表, 子の年齢たち) {
    var u = 表.university;
    var 多子 = (子の人数 >= (u.multi_child_min_children || 3));
    var 年齢 = (子の年齢たち && 子の年齢たち.length) ? 子の年齢たち : null;
    if (!年齢) {
      年齢 = [];
      for (var k = 0; k < Math.max(1, 子の人数 || 1); k++) { 年齢.push(k === 0 ? 18 : 17); }
    }
    var 所得 = 給与所得(年収, 手取りの年分);
    var 社保 = 社会保険料(Math.max(0, Math.floor(年収 || 0)));
    /* 扶養控除（住民税）と、調整控除に使う人的控除の差 */
    var 扶養控除 = 0, 控除差 = 50000 /* 基礎 */ + 50000 /* ひとり親 */;
    年齢.forEach(function (a) {
      if (a >= 19 && a <= 22) { 扶養控除 += 450000; 控除差 += 180000; }
      else if (a >= 16 && a <= 18) { 扶養控除 += 330000; 控除差 += 50000; }
    });
    var 算定額;
    if (所得 <= 住民税表.ひとり親等の非課税限度) {
      算定額 = 0;   /* 住民税がかからない人は、0円とみなす */
    } else {
      var 課税標準 = Math.max(0, Math.floor((所得 - 社保 - 住民税表.基礎控除 - 住民税表.ひとり親控除 - 扶養控除) / 1000) * 1000);
      var 調整控除 = (課税標準 <= 2000000)
        ? Math.min(控除差, 課税標準) * 0.03
        : Math.max(控除差 - (課税標準 - 2000000), 50000) * 0.03;
      算定額 = Math.max(0, Math.floor((課税標準 * 0.06 - 調整控除) / 100) * 100);
    }
    var 割合 = 0;
    if (算定額 < 修学支援の基準.第1) { 割合 = 1; }
    else if (算定額 < 修学支援の基準.第2) { 割合 = 0.6667; }
    else if (算定額 < 修学支援の基準.第3) { 割合 = 0.3333; }
    else if (多子 && 算定額 < 修学支援の基準.第4) { 割合 = (u.multi_child_tier ? u.multi_child_tier.ratio : 0.25); }
    return { ratio: 割合, multiChild: 多子, amount: 算定額 };
  }
  ...
  function 学費の支援(年齢, プラン, 学費表, 状況) {
    ...
    if (帯.stage === 'high') {
      ...
      /* 就学支援金は、ここでは引かない。
         学習費調査の金額は「保護者が実際に払った額」で、支援金の分はすでに引かれているため。
      ...
      var 判定 = 大学の支援割合(状況.income || 0, 状況.children || 0, 支, 状況.ages);
  ...
  function その年の学費(子の年齢たち, プラン一覧, 学費表, 状況, 塾設定) {
    ...
    (子の年齢たち || []).forEach(function (age, i) {
      ...
      /* 大学の支援の区分は、きょうだいの年齢（扶養控除）で変わるので、その年の年齢も渡す */
      var 状況と年齢 = 状況 ? Object.assign({}, 状況, { ages: 子の年齢たち }) : null;
      var 支 = 状況 ? Math.min(内.school, 学費の支援(age, プラン, 学費表, 状況と年齢)) : 0;
      if (内.total > 0) {
  ...
  /** 全部一番安い道（すべて公立・大学は国公立で自宅から）を選んだときのプラン */
  function 一番安いプラン(学費表) {
    var p = {};
  ...
  function 保育料(子の年齢たち, 年収, ひとり親か, 表) {
    /* 前に計算したときの内訳を残さない（0円の年に、前の年の内訳が出てしまうため） */
    保育料.内訳 = { total: 0, gross: 0, discount: 0, detail: [] };
    ...
    (子の年齢たち || []).forEach(function (age, i) {
      ...
      /* 未就学の子のなかで、上から何番目か。
         双子など同じ年齢の子は、入力の順に1人ずつ数える（同じ順位にしない）。 */
      var 同い年で先 = (子の年齢たち || []).slice(0, i)
        .filter(function (a) { return a === age; }).length;
      var 順位 = 未就学.indexOf(age) + 1 + 同い年で先;
      var 割合 = 1;
      if (低所得ひとり親) {
        割合 = (順位 >= 2) ? 0 : 1;
      } else if (順位 >= 3) {
  ...
  function 奨学金の返済(貸与総額, 表) {
    ...
    /* 利子がつく場合（第二種）。元利均等でおよその額を出す。
       卒業してから返し始めるまでの6か月分の利子も元本に乗るので、
       その分も見込んでおく（機構の返還例に近づけるため）。 */
    function 利子つき月額(年利) {
      if (!(年利 > 0)) { return 無利子月額; }
      var 月利 = 年利 / 12;
      var 据置 = 6;   // 貸与が終わってから返し始めるまでの月数分の利子
  ...
  function 資産カーブ(入力, データ) {
    ...
    /* ひとり親控除で軽くなる税（1か月あたり）。カードと同じ1本の関数を使う */
    var 控除の効果 = Math.floor(ひとり親控除の効果(入力.myIncome, 子の人数).total / 12);
    ...
    /* 月ごとの並び。一番最初（month 0）は「いまの貯金」そのもの。
    ...
    var points = [], 学費の合計 = 0, 一番安い学費の合計 = 0;
    var 学費の総額 = 0, 学費の支援合計 = 0;
    var 大学で赤字 = null;
    var 安いプラン = 学費表 ? 一番安いプラン(学費表) : null;

    /* 【グラフの点の意味】
       points[i] は「一番下のお子さんが（末子+i）歳になった時点」の貯金です。
       だから points[0] は、入力していただいた貯金額そのもの（まだ1円も足していない状態）。
       そこから1年分足したものが points[1]、という並びにします。
       （先に足してから点を置くと、グラフの左端が1年後の残高になってしまうため） */
    sim.years.forEach(function (y, i) {
      var d = y.divorced;

      /* この年の、お子さん全員分の学校のお金。
         学費を助ける制度は、収入が低い世帯ほど手厚い。
         これを入れないと「私立に行ったら終わり」という、実際と違う絵になる。
      ...
      /* 家計の内訳。「なぜこの年に落ちるのか」を、あとから確かめられるようにする。
         手取りには、ひとり親控除の分がすでに入っているので、いったん外に出しておく。 */
      ...
      function 内訳(全部か) {
        ...
        var 収入 = [
          ...
          { key: 'kojo', name: 'ひとり親控除で税が軽くなる分', amount: 控除,
            reason: (控除 === 0 ? (控除が使える ? 'まだ申告していない' : '所得が500万円を超えているため対象外') : null) },
      ...
      points.push({
        ...
        monthlyNow: 月収支いま,
        monthlyAll: 月収支全部,
        breakdown: { all: 内訳(true), now: 内訳(false) },
        now: 貯金いま,
        all: 貯金全部
      });

      /* 一番最後の点は「末子22歳の時点」なので、そこから先は積まない。
    ...
    return {
      ...
      tuitionCheapestTotal: 一番安い学費の合計,
      tuitionExtra: 学費の合計 - 一番安い学費の合計
    };
  }
  ...
  function 資格ルート(入力, データ, sim) {
    ...
    sim.years.forEach(function (y, i) {
      ...
      var 手取り月 = Math.floor(手取り目安(年収, true) / 12);
      ...
      /* こちらも、貯めるより先に点を置く。
         一番左の点は「いまの貯金」で、3本の線がそこから分かれる形にする。 */
      points.push({
        ...
        breakdown: {
          income: [
            { key: 'takehome', name: '手取り（' + (訓練中 ? '学校に通いながら働く分' : '資格を取ったあと') + '）',
              amount: 手取り月 },
            ...
            { key: 'jidoFuyoTeate', name: '児童扶養手当', amount: 児扶.monthly,
              reason: (児扶.monthly === 0 ? '所得が限度額を超えているため対象外' : null) },
            { key: 'jidoTeate', name: '児童手当', amount: 児手,
    ...
    /* 「いまのまま」を追い越す年。
       一番左の点は3本とも同じ（いまの貯金）なので、そこは数えない。 */
    var 逆転 = null;
      ...
      switch (p.id) {
        ...
        case 'jido_fuyo_teate': {
          ...
          if (j.status === 'none') { r.status = 'unlikely'; r.label = '対象外の見込み'; r.note = j.reason || ''; }
            ...
            r.amountText = '1か月あたり およそ ' + 円(j.monthly) + '（年およそ ' + 円(j.monthly * 12) + '）';
        ...
        case 'jido_teate': {
          ...
          if (h.monthly > 0) {
            ...
            r.amountText = '1か月あたり ' + 円(h.monthly) + '（年 ' + 円(h.monthly * 12) + '）';
        ...
        case 'hitorioya_kojo': {
          ...
          if (入力.isSingleParent && 合計所得 <= e.income_ceiling && (入力.children || []).length > 0) {
            r.status = 'likely'; r.label = '対象の可能性が高い';
            var 軽さ = ひとり親控除の効果(入力.myIncome, (入力.children || []).length);
            var 効果 = 軽さ.total;
            ...
            if (効果 === 0) {
              ...
              r.note = 'いまの収入では、この控除がなくても、もともと所得税・住民税がかかりません。'
                + 'そのため、控除で戻ってくるお金は0円です。'
                + 'それでも申告しておく意味があります。ひとり親であることが記録に残ると、'
                + '合計所得が135万円以下の間は住民税がかからない扱いになり（地方税法第295条第1項第2号）、'
                + '収入が増えたときにも自動でこの控除が効くようになります。'
                + '保育料や高校・大学の支援は住民税の額で決まるものが多いので、'
                + '申告しておいて損になることはありません。'
                + '（申告先は、勤め先の年末調整、または確定申告です）';
            }
            ...
            r.status = 'unlikely'; r.label = '対象外の見込み';
            r.note = '合計所得が ' + 円(e.income_ceiling) + ' を超えていると使えません。';
          }
          break;
        }
