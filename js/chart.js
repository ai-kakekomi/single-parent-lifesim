...
(function (root, factory) {
   ...
   * @param {String} 見方   'perPerson'（1人あたりに直した金額。ふだんはこちら）
   *                        または 'total'（家全体の金額）
   */
  function 描く(years, cliffs, 見方, 縦長) {
    ...
    if (縦長) {
      ...
      /* パソコンでも、縦を従来より伸ばす（320→430、約1.34倍）。
         ...
         画面に収まる範囲でとどめる。 */
      /* 本文の幅（720px）を超えると、横スクロールが必要になってしまう */
      W = Math.min(720, Math.max(500, 70 + years.length * 42)); H = 430;
      左 = 62; 右 = 66; 上 = 24; 下 = 58;
    }
    var 幅 = W - 左 - 右, 高 = H - 上 - 下;
    ...
    s.push('<svg viewBox="0 0 ' + W + ' ' + H + '" width="' + W + '" height="' + H +
      '" role="img" aria-label="結婚を続けた場合と離婚した場合の、1か月あたりのお金の推移">');
    ...
    /* --- 制度の崖（縦線）。名前はグラフの外の一覧に出し、線には番号だけ置く。
    ...
    s.push('<text x="' + (左 + 幅 / 2) + '" y="' + (H - 18) + '" text-anchor="middle" font-size="12" fill="' + 色.sub + '">一番下のお子さんの年齢（歳）</text>');
    s.push('<text x="' + 左 + '" y="' + (上 - 8) + '" font-size="12" fill="' + 色.sub + '">' +
      (値 === 'total' ? '家賃を払ったあと、1か月に残るお金（家全体）' : '家賃を払ったあと、1か月に残るお金（1人あたり）') + '</text>');
  ...
  function 表(years, 見方) {
    ...
    var 値 = (見方 === 'total') ? 'total' : 'perPerson';
    var 見出し = (値 === 'total') ? '家全体の金額' : '1人あたりに直した金額';
    ...
    var h = ['<table class="compare"><caption class="hint" style="text-align:left">' + 見出し +
      '（1か月あたり）</caption><thead><tr><th>一番下の子</th><th>結婚を続けた場合</th><th>離婚した場合</th><th>差</th></tr></thead><tbody>'];
  ...
  function 資産を描く(curve, 縦長, カーソル年) {
    ...
    var W, H, 左, 右, 上, 下;
    if (縦長) {
      /* 上には「貯まっていく貯金」と「いまの貯金 ◯◯円」の2行が入る。
         その2行分の高さをとっておかないと、上が切れて読めなくなる。 */
      W = 360; H = 470;
      左 = 56; 右 = 60; 上 = 46; 下 = 56;
    } else {
      /* 横に長くしすぎると、画面からはみ出して横スクロールが必要になる。
         本文の幅（760px から左右の余白を引いた 720px）を超えないようにする。 */
      W = Math.min(720, Math.max(500, 70 + pts.length * 42)); H = 430;
      左 = 66; 右 = 74; 上 = 24; 下 = 58;
    }
    var 幅 = W - 左 - 右, 高 = H - 上 - 下;
    ...
    /* 横軸の右端は「一番先まで描く線の終わり＋少しの余白」まで。
       描かない先まで軸を伸ばすと、肝心の部分が左に押しつぶされて読めなくなるため。
       資格ルートを出して線が先まで伸びれば、軸もその分広がる。 */
    ...
    /* 生活防衛資金の線は、グラフには描かない。
       ...
       線が1本増えるとそれが読みにくくなるため。考え方は、グラフの下の文章で伝えている。 */
    ...
    s.push('<text x="' + (左 + 幅 / 2) + '" y="' + (H - 18) + '" text-anchor="middle" font-size="12" fill="' + 色.sub + '">一番下のお子さんの年齢（歳）</text>');
    s.push('<text x="' + 左 + '" y="' + (縦長 ? 上 - 30 : 上 - 8) + '" font-size="12" fill="' + 色.sub + '">貯まっていく貯金</text>');
    ...
    /* --- 資格を取るルート --- */
    /* 線の端の名前は、一番下（床のラベルを置く帯）には入れない */
    var 下限y = 上 + 高 - 24;
    function 収める(v) { return Math.min(Math.max(v, 上 + 10), 下限y); }
    /* 線の端の名前は、その線が実際に終わる場所に置く */
    function はし(key) {
